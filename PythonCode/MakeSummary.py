...
for idx, color in enumerate(COLORMAP):
    ...
    current_strnrate, = AX1.semilogy(FDEN, STRNRATE, color=color,
                                     ...
                                     alpha=1)
    current_pcon = AX1A.plot(FDEN, PCON, color='green', label=LBL_PCON,
                             linestyle='-',
                             linewidth=2, marker='o', markersize=6,
                             markerfacecolor='green', fillstyle='none',
                             alpha=1)
    current_pcon = AX1A.plot(FDEN, PPOR, color='blue', label=LBL_PPOR,
                             linestyle='-',
                             linewidth=2, marker='o', markersize=6,
                             markerfacecolor='blue', fillstyle='none',
                             alpha=1)
    current_pcon = AX1A.plot(FDEN, PTER, color='red', label=LBL_PTER,
                             linestyle='-',
                             linewidth=2, marker='o', markersize=6,
...
AX1.set_xlim(xmin=0.94, xmax=0.955)
...
AX1A.tick_params(labelsize=FS, pad=10)

HANDLES_1, LABELS_1 = AX1.get_legend_handles_labels()
#  bbox_to_anchor(xmin, ymin, xmax, ymax) ro bounding box of legend
LGD1 = AX1.legend(HANDLES_1, LABELS_1, bbox_to_anchor=(1.1, 0.4, 1.03, 1.0),
                  loc='lower left', borderaxespad=1, ncol=1,
                  fontsize=10)
HANDLES_1A, LABELS_1A = AX1A.get_legend_handles_labels()
LGD1A = AX1A.legend(HANDLES_1A, LABELS_1A,
                    bbox_to_anchor=(1.1, 0.0, 1.03, 0.4),
...
FIG1.savefig(FIG1_PATH, bbox_extra_artists=(LGD1A,), bbox_inches='tight')
