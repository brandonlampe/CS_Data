...
# calculated volume strain rate
FDEN0 = FDEN_INTERP[0]  # INITIAL FRACTIONAL DENSITY
# VSTRN = -np.log(FDEN0 / FDEN_INTERP)  # "TRUE" VOL. STRN, + IN COMPRESSION
# # np.gradient() uses a central difference for interior nodes
# DELTA_VSTRN = np.gradient(VSTRN)  # incremental change in Vol. strn
# DELTA_SEC = np.gradient(DUR_SEC)  # incremental time step
# DELTA_SEC_INTERP = np.gradient(DUR_SEC_INTERP)  # incremental time step
# VSTRN_RATE = DELTA_VSTRN / DELTA_SEC_INTERP  # SEC^-1

# FIT PARAMETER VALUES TO SPECIFIED TEST DATA
MODEL = daf.fit_fden(FDEN_INTERP, DUR_DAY_INTERP)
...
AXARR[0].plot(DUR_DAY, FDEN, linestyle='-', linewidth=1,
              marker='.', markersize=12, color='r', alpha=1)
AXARR[0].plot(DUR_DAY_INTERP, FDEN_FIT_INTERP, linestyle='-',
              linewidth=3, marker=None, markersize=1, color='b',
              alpha=1)
AXARR[0].grid(True)
...
AXARR[0].tick_params(labelsize=FS, pad=10)
#################################
lbl_strnrate = ["Interpolated", "Measured", "Fit"]
...
AXARR[1].semilogy(DUR_DAY_INTERP, VSTRN_RATE_FIT_INTERP, linestyle='-',
                  linewidth=3, marker='None', markersize=12,
                  color='b', alpha=1)
AXARR[1].grid(True)
...
AXARR[1].legend(lbl_strnrate, frameon=1, framealpha=0.75, loc=3, fontsize=FS)
...
# FIG1.tight_layout()
FIG1_NAME = ("PLOTS_" + TESTNAME + "_" + str(DUR_START) + "-" +
             str(DUR_END) + '.pdf')
...
if SAVECSV != 0:
    ...
    print("File Saved As: " + OUT_FILENAME)
