\documentclass[oneside, 12pt, letterpaper]{article}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage{graphicx}
\graphicspath{{figs/}{../figs/}}
\usepackage{outlines}  % loads package for outline
\usepackage{enumitem}  % allows for custom enumeration on items
\usepackage[margin=1in]{geometry}
\usepackage{bm}  % bold math symbols
\usepackage{mathtools}

\usepackage{blindtext} % option to remove section number using {}*
\usepackage[colorlinks=true,linkcolor=blue,citecolor=blue]{hyperref}  % automatic hyper links
\usepackage{multirow}
\usepackage{breqn}
\usepackage{titlesec}
\usepackage{mathrsfs}  % for script math fonts \mathscr{}
\usepackage{deluxetable}
\usepackage{pdflscape}

\setenumerate[1]{label=\Roman*.}
\setenumerate[2]{label=\Alph*.}
\setenumerate[3]{label=\roman*.}
\setenumerate[4]{label=\alph*.}

\newcommand{\eqns}[1]{\begin{equation*}#1\end{equation*}}
\newcommand{\eqn}[1]{\begin{equation}#1\end{equation}}
\newcommand{\as}[1]{\begin{align*}#1\end{align*}}
\newcommand{\an}[1]{\begin{align}#1\end{align}}
\newcommand{\bdd}{\begin{dd}}
\newcommand{\edd}{\end{dd}}
\newcommand{\bi}{\begin{itemize}}
\newcommand{\ei}{\end{itemize}}
\newcommand{\be}{\begin{enumerate}}
\newcommand{\ee}{\end{enumerate}}

 \newcommand{\Figure}[3]{
  \begin{figure}[!htp]
    \centering
    \includegraphics[width=#1in]{#2}
    \caption{#3.}
  \end{figure}}
\newcommand\degc{${^\circ}C$}

\author{Brandon Lampe}
\title{Summary of: UNM 175 09}
\date{\today}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% BEGIN THE DOCUMENT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
% \maketitle

% \begin{landscape}

% \begin{rotatetable}
\begin{deluxetable}{lrccccccccc}
\rotate
\tablewidth{0pc}
\tablecolumns{11}
\tablecaption{Summary of results from \texttt{UNM_WP_HY_175_09} \label{tab:results}}
\tablehead{\colhead{Stage}                  &
           \colhead{Time Start}             &
           \colhead{Time End}               &
           \colhead{Duration}               &
           \colhead{Initial $\rho_f$}       &
           \colhead{Final $\rho_f$}         &
           \colhead{Initial $\dot{e}$}      &
           \colhead{Final $\dot{e}$}        &
           \colhead{$P_{con}$}              &
           \colhead{$P_{por}$}              &
           \colhead{Temperature}            \\
           \colhead{}                       &
           \colhead{(days)}                 &
           \colhead{(days)}                 &
           \colhead{(days)}                 &
           \colhead{}                       &
           \colhead{}                       &
           \colhead{$\left(sec^{-1}\right)$}&
           \colhead{$\left(sec^{-1}\right)$}&
           \colhead{(MPa)}                  &
           \colhead{(MPa)}                  &
           \colhead{(\degc)}                     }
\startdata
1  & 0.00650 & 0.975 & 0.969 & 0.832 & 0.886 & 3.48$\times 10^{-5}$ & 1.53$\times 10^{-7}$ & 13.9 & -0.0776 & 172\\
2  & 0.980 & 2.02 & 1.04 & 0.887 & 0.894 & 1.70$\times 10^{-7}$ & 5.96$\times 10^{-8}$ & 22.0 & 7.89 & 171\\
3  & 2.02 & 2.95 & 0.928 & 0.895 & 0.909 & 6.15$\times 10^{-7}$ & 7.58$\times 10^{-8}$ & 22.0 & 4.03 & 170\\
4  & 2.96 & 3.95 & 0.990 & 0.911 & 0.920 & 3.54$\times 10^{-7}$ & 6.34$\times 10^{-8}$ & 22.0 & -0.0840 & 170\\
5  & 3.99 & 5.00 & 1.01 & 0.920 & 0.922 & 3.87$\times 10^{-8}$ & 3.97$\times 10^{-9}$ & 30.0 & 7.91 & 170\\
6  & 5.00 & 5.99 & 0.985 & 0.923 & 0.928 & 5.10$\times 10^{-7}$ & 4.17$\times 10^{-8}$ & 30.0 & 4.02 & 170\\
7  & 6.02 & 6.91 & 0.890 & 0.928 & 0.932 & 1.64$\times 10^{-7}$ & 2.51$\times 10^{-8}$ & 30.0 & -0.0832 & 170\\
8  & 7.11 & 8.11 & 0.996 & 0.932 & 0.934 & 2.74$\times 10^{-8}$ & 2.74$\times 10^{-8}$ & 38.0 & 7.98 & 170\\
9  & 8.11 & 9.03 & 0.920 & 0.935 & 0.936 & 5.76$\times 10^{-8}$ & 5.01$\times 10^{-9}$ & 38.0 & 4.01 & 170\\
10 & 9.05 & 10.9 & 1.84 & 0.936 & 0.939 & 6.59$\times 10^{-8}$ & 1.28$\times 10^{-8}$ & 38.0 & 0.106 & 170\\
11 & 11.0 & 11.9 & 0.953 & 0.941 & 0.941 & 5.83$\times 10^{-9}$ & 5.82$\times 10^{-9}$ & 38.0 & 7.94 & 170\\
12 & 12.0 & 13.3 & 1.29 & 0.942 & 0.942 & 7.30$\times 10^{-9}$ & 7.29$\times 10^{-9}$ & 38.0 & -0.0573 & 170\\
13 & 13.3 & 15.0 & 1.70 & 0.943 & 0.943 & 4.99$\times 10^{-9}$ & 4.99$\times 10^{-9}$ & 38.0 & 8.03 & 170\\
14 & 15.0 & 15.9 & 0.927 & 0.941 & 0.941 & -2.41$\times 10^{-11}$ & -2.41$\times 10^{-11}$ & 22.0 & 8.06 & 170\\
15 & 16.1 & 19.1 & 2.95 & 0.941 & 0.942 & 1.02$\times 10^{-9}$ & 1.02$\times 10^{-9}$ & 22.0 & -0.0846 & 170\\
16 & 19.1 & 20.2 & 1.03 & 0.944 & 0.944 & 6.71$\times 10^{-9}$ & 6.71$\times 10^{-9}$ & 38.0 & 7.95 & 170\\
17 & 20.4 & 22.0 & 1.62 & 0.944 & 0.945 & 5.47$\times 10^{-9}$ & 5.47$\times 10^{-9}$ & 38.0 & -0.0771 & 170\\
18 & 23.2 & 26.0 & 2.75 & 0.945 & 0.946 & 3.29$\times 10^{-9}$ & 3.28$\times 10^{-9}$ & 38.0 & 7.99 & 170\\
19 & 44.5 & 47.5 & 3.00 & 0.950 & 0.951 & 2.28$\times 10^{-9}$ & 2.28$\times 10^{-9}$ & 37.9 & -0.0790 & 171\\
20 & 51.5 & 55.0 & 3.50 & 0.951 & 0.952 & 2.23$\times 10^{-9}$ & 2.23$\times 10^{-9}$ & 37.9 & -0.100 & 170\\
21 & 58.2 & 62.0 & 3.75 & 0.952 & 0.953 & 1.31$\times 10^{-9}$ & 1.31$\times 10^{-9}$ & 37.8 & -0.0931 & 170\\
22 & 62.1 & 69.0 & 6.90 & 0.953 & 0.954 & 1.72$\times 10^{-9}$ & 1.71$\times 10^{-9}$ & 37.8 & -0.0959 & 170\\
\enddata
\end{deluxetable}
% \end{rotatetable}

% \end{landscape}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% END DOCUMENT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
